...
import "./CalmMood.css"; // Reuse the same styling

const ReflectiveMoodPage = () => {
  const [mood, setMood] = useState(null);
  const [songs, setSongs] = useState([]);
  const [error, setError] = useState("");

  // Step 1: Fetch the Reflective mood
  useEffect(() => {
    const fetchMood = async () => {
      try {
        ...
        const reflective = moods.find((m) => m.name.toLowerCase() === "reflective");

        if (reflective) {
          setMood(reflective);
        } else {
          setError("Reflective mood not found.");
        }
      } catch (err) {
  ...
  // Step 2: Fetch songs for the Reflective mood
  useEffect(() => {
    if (!mood) return;
...
export default ReflectiveMoodPage;
